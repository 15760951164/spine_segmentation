...
from scipy.ndimage import center_of_mass, binary_erosion, label, zoom, gaussian_filter
from models.UNet import *
from models.SCN_fuse import SCN
from transformations.intensity.np.normalize import normalize
...
class LandmarkGraphOptimization(object):
    ...
    def __init__(self, num_landmarks, bias=3.0, l=0.3):
        ...
        self.bias = bias
        self.l = l
        with open('pickle/units_distances.pickle', 'rb') as f:
            self.offsets_mean, self.distances_mean, self.distances_std = pickle.load(f)
    ...
    def pairwise_term(self, landmark_a, landmark_b, a_index, b_index):

        # distance_value = np.linalg.norm(landmark_a.coords - landmark_b.coords)
        distance_value = self.distance_value(landmark_a, landmark_b, a_index, b_index)
        ...
        for curr in range(len(new_local_heatmap_maxima) - 1):
            ...
            for curr_index, curr_landmark in enumerate(curr_landmarks):
                for next_index, next_landmark in enumerate(next_landmarks):
                    ...
                    weight = -self.pairwise_term(curr_landmark, next_landmark, curr, next) - self.unary_term(curr_landmark)
    ...
    def graph_optimization(self, local_heatmap_maxima, graph_filename=None):
        ...
        try:
            ...
            return False, local_heatmap_maxima
        ...
        return True, self.path_to_landmarks(shortest_path, local_heatmap_maxima)
...
class InferenceBase(object):
    ...
    def load_model(self):
        ...
        if hasattr(self.model_func, 'module'):
            self.model_func.module.load_state_dict(checkpoint['state_dict'])
        else:
            self.model_func.load_state_dict(checkpoint['state_dict'])
            
        if torch.cuda.is_available():
            self.model_func.to(torch.device(self.device))
...
class VertebraLocate_Base(InferenceBase):
    ...
    def inference_roi_image(self, image: np.ndarray, roi_box_ori: Box,
        ...
        vol_output = np.zeros(
            (self.num_labels, image.shape[0], image.shape[1], image.shape[2])).astype(np.float32)
    ...
    def inference_heatmap(self, inference_image=None, roi_box: Box = None):
        ...
        heatmaps = zoom(heatmaps, zoom=(2, 2, 2), order=1)
        # vol_output = zoom(vol_output, zoom=(1, 2, 2, 2), order=1)
...
class VertebraLocate_25_Channel(VertebraLocate_Base):
    ...
    def __init__(
        ...
        model_file=None,
        model_func=SCN(in_channels=1, out_channels=25, f_maps=64)
    ) -> None:
    ...
    def inference_aux(self, image, roi_box):
        ...
        for i in range(self.num_labels):
            ...
            if np.max(vol) < self.min_landmark_value:
                coords_list.append([
            ...
            vol_copy = np.copy(vol)
            vol_copy = gaussian_filter(vol_copy, sigma=1.0)
            # th = filters.threshold_otsu(vol_copy)
            vol_copy = np.clip(vol_copy, self.min_landmark_value, np.max(vol_copy))
    ...
    def get_mean_coords(self, coords_info):
        mean_coords = []
        for coord_list in coords_info:
            if len(coord_list) != 1:
                ...
                mean_coords.append(
                    Landmark(coords=np.stack(m_coord_list, axis=0).mean(axis=0).astype(np.int32),
                             is_valid=True,
                             scale=1.0,
                             label=coord_list[0].label,
                             value=np.stack(m_value_list, axis=0).mean(axis=0)))
            else:
                mean_coords.append(coord_list[0])
        return mean_coords
    ...
    def graph_optimization(self, coords_info, vol_input):
        ...
        states, coords_info_p = postprocess.graph_optimization(coords_info,
                                                     graph_filename=None)

        if states == False:
            return self.get_mean_coords(coords_info)
            
        return coords_info_p
    ...
    def inference(self,
        ...
        if self.postprocess is not None:
            ...
            coords_info = self.get_mean_coords(coords_info)
        
        #coords_info = self.filter_landmarks_top_bottom(coords_info, vol_input.shape)
        coords_info = self.remove_valid_landmark(coords_info)
        
        coords_info_2mm = copy.deepcopy(coords_info)
        coords_info_1mm = self.convert_coords_to_1mm(coords_info)
        ...
        return heatmap, coords_info_1mm, coords_info_2mm
...
class VertebraSegmention(InferenceBase):
    ...
    def inference(self,
        ...
        for index, landmark in enumerate(coord_info):
            ...
            x_start, x_end = x - self.cube_size[0] // 2, x + self.cube_size[0] // 2
            y_start, y_end = y - self.cube_size[1] // 2, y + self.cube_size[1] // 2
            z_start, z_end = z - self.cube_size[2] // 2, z + self.cube_size[2] // 2

            if x_end > h or y_end > w or z_end > c or z_start < 0:
                continue

            self.display_info(
                f"idv_segment at coords {index + 1}/{len(coord_info)}_{landmark.label} --> {coords}...", step=1)

            crop_image = image_pad[x_start:x_end, y_start:y_end, z_start:z_end]

            assert (np.array(crop_image.shape) == np.array(self.cube_size)).all()

            heatmap = generator_one_heatmap(
                    heatmap_size=self.cube_size, 
                    coords=[self.cube_size[0]//2, self.cube_size[1]//2, self.cube_size[2]//2], 
                    sigmas=self.sigmas
                )
...
class VertebraLabelClassifier(InferenceBase):
    ...
    def load_model(self):
        for model_name, model_path in self.model_file.items():
            ...
            cur_model = self.model_func(n_classes=self.get_classifier_type(model_name))
            state_dict = torch.load(model_path)["state_dict"]
            cur_model.load_state_dict(state_dict)
            if torch.cuda.is_available():
                cur_model.to(torch.device(self.device))
            cur_model.eval()
