...
def generate_heatmap_target(heatmap_size,
    ...
    squared_distances = np.sum(np.power(grid_stacked - landmarks_reshaped, 2.0), axis=-1, dtype=np.float32)
    heatmap = scale * np.exp(-squared_distances /(2 * np.power(sigmas_reshaped, 2)), dtype=np.float32)
    heatmap_or_zeros = np.where((is_valid_reshaped + np.zeros_like(heatmap)) > 0, heatmap, np.zeros_like(heatmap, dtype=np.float32))
    
    heatmap_or_zeros_sum = np.sum(heatmap_or_zeros, axis=0, dtype=np.float32)
